  ...
  async function refresh(){
    ...
    try{
      ...
      list.innerHTML=`<div class="occ-stats"><div><b>${d.totalUnits}</b><span>UNIT TERHUNI</span></div><div><b>${d.totalPax}</b><span>PAX TERHUNI</span></div></div><div class="occ-title">REKAP PER TOWER</div>`+
        (towers.length?towers.map(t=>{const x=d.tower[t];return `<details class="occ-tower"><summary><strong>TOWER ${esc(String(t).padStart(2,'0'))}</strong><span>${x.units} Unit · ${x.pax} Pax</span></summary><div>${x.rows.map(r=>`<div class="occ-unit"><div><strong>T${esc(String(r.tower).padStart(2,'0'))}-${esc(r.unit_number)}</strong><span>${r.pax} Pax</span></div><small>${r.names.map(esc).join(' · ')}</small></div>`).join('')}</div></details>`}).join(''):'<div style="padding:25px;text-align:center;color:#687386">Belum ada data penghuni.</div>');
    }catch(e){
