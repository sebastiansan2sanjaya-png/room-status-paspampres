...
(function(){
  ...
  async function load(){
    ...
    const [u,g]=await Promise.all([
      window.supabaseClient.from('room_units').select('id,tower,floor,unit_number,status').eq('status','OD').order('tower').order('floor').order('unit_number'),
      window.supabaseClient.from('room_guests').select('unit_id,guest_order,guest_name').not('guest_name','is',null)
    ]);
    if(u.error) throw u.error; if(g.error) throw g.error;
    units=u.data||[]; guests=(g.data||[]).filter(x=>String(x.guest_name||'').trim());
    return {units,guests};
  }
  function build(){
    const byUnit={}; guests.forEach(g=>(byUnit[g.unit_id]??=[]).push(g));
    const tower={}; units.forEach(u=>{const pax=(byUnit[u.id]||[]).length;(tower[u.tower]??={units:0,pax:0,rows:[]});tower[u.tower].units++;tower[u.tower].pax+=pax;tower[u.tower].rows.push({...u,pax,names:(byUnit[u.id]||[]).sort((a,b)=>a.guest_order-b.guest_order).map(x=>x.guest_name)});});
    const totalUnits=units.length,totalPax=guests.filter(g=>units.some(u=>u.id===g.unit_id)).length;
    return {byUnit,tower,totalUnits,totalPax};
  }
  function open(){
    const modal=document.getElementById('occupancyRecapModal'); if(modal) modal.style.display='flex'; refresh();
  }
